/*
  # Badge Auto-Awarding System (CORRECTED)
  ...
  4. CORRECTIONS MADE:
    ...
    - modifications table: Uses `user_id` (matches schema)
    - spot_history table: Uses `spotter_id` (confirmed from code usage)
...
-- Create the badge auto-award function with CORRECTED table/column names
CREATE OR REPLACE FUNCTION check_and_award_badges(p_user_id UUID, p_action TEXT)
RETURNS TABLE(badge_id TEXT, badge_name TEXT, badge_category TEXT) AS $$
DECLARE
...
BEGIN
  ...
  CASE p_action
    WHEN 'spot' THEN
      -- CORRECTED: spot_history table exists and uses spotter_id
    ...
    WHEN 'photo' THEN
      -- CORRECTED: spot_history with photo_url check
      SELECT COUNT(*) INTO activity_count
      FROM spot_history
      WHERE spotter_id = p_user_id
    ...
    WHEN 'mod' THEN
      -- VERIFIED: modifications table uses user_id
      SELECT COUNT(*) INTO activity_count
      FROM modifications
      WHERE user_id = p_user_id;
  ...
  LOOP
    ...
    badge_category := badge_rec.category;
    RETURN NEXT;
  END LOOP;
...
COMMENT ON FUNCTION check_and_award_badges IS 'Automatically checks user progress and awards appropriate tiered badges. Corrected to use actual table and column names from schema.';
