  ...
  return (
    <Layout currentPage="scan" onNavigate={onNavigate}>
      <div style={{ background: '#030508', minHeight: '100vh', paddingBottom: 100 }}>
        ...
        {/* Unlocked spec block — VIN data exists */}
        {hasVinData && specRows.length > 0 && (
        ...
        {/* Locked spec block — no VIN data */}
        {!hasVinData && (
          <div style={{ margin: '14px 16px 0', padding: '14px 16px', background: '#0d1117', border: '1px solid rgba(255,255,255,0.05)', borderRadius: 10 }}>
            <div style={{ fontFamily: "'Barlow Condensed', sans-serif", fontSize: 8, fontWeight: 700, letterSpacing: '0.2em', textTransform: 'uppercase' as const, color: '#5a6e7e', marginBottom: 6 }}>
              Factory Specs
            </div>
            <div style={{ filter: 'blur(4px)', pointerEvents: 'none' as const }}>
              {['Engine', 'Horsepower', 'Drivetrain', 'Transmission'].map((label, i) => (
                <div key={label} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '10px 0', borderBottom: i < 3 ? '1px solid rgba(255,255,255,0.04)' : 'none' }}>
                  <span style={{ fontFamily: "'Barlow Condensed', sans-serif", fontSize: 9, fontWeight: 700, letterSpacing: '0.14em', textTransform: 'uppercase' as const, color: '#5a6e7e' }}>{label}</span>
                  <span style={{ fontFamily: "'JetBrains Mono', monospace", fontSize: 12, color: '#eef4f8' }}>{'\u25cf \u25cf \u25cf \u25cf'}</span>
                </div>
              ))}
            </div>
            <div style={{ fontFamily: "'Barlow', sans-serif", fontSize: 11, color: '#7a8e9e', marginTop: 10, textAlign: 'center' as const }}>
              Run a Verified Spot to unlock factory specs
            </div>
          </div>
        )}
