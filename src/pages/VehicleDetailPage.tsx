...
import { ArrowLeft, Edit, Trash2, AlertCircle, Upload, X, Star, Shield, Key, Info, Share2, ChevronLeft, User, Wrench, Disc3, Palette, Armchair, Droplet, FileText, Download, Car, MapPin, MoreHorizontal, Camera, BookOpen, ChevronRight, ChevronDown, Heart, Circle, Check } from 'lucide-react';
...
import { getVehicleClaimStatus, getVehicleImage, formatRP, formatCount, formatSpotTime, getVehicleDisplayName } from '../lib/vehicleUtils';
...
export function VehicleDetailPage({ vehicleId, onNavigate, onBack, onEditBuildSheet, guestMode = false, scrollTo, openReviewModal }: VehicleDetailPageProps) {
  ...
  const { isTracking, trackerCount, toggle: toggleTracking } = useVehicleTracking(vehicle?.id);
  const claimResult = vehicle ? getVehicleClaimStatus(vehicle) : { status: 'unclaimed' as const, label: 'UNCLAIMED' as const };
  ...
  const loadVehicleData = async () => {
    ...
    // Fetch spots/reviews from posts table
  ...
  const handleToggleHidden = async (review: Review) => {
    // Toggle visibility — not supported on posts table, remove from view instead
    const { error: deleteError } = await supabase
      .from('posts')
      .delete()
      .eq('id', review.id);
    ...
    <Layout currentPage="profile" onNavigate={onNavigate}>
      ...
      <div className="pb-24 animate-page-enter">
        ...
        <div className="relative w-full stg" style={{ minHeight: 200 }}>
          {(() => {
            const heroUrl = vehicleImages[0]?.image_url || getVehicleImage(vehicle, 'profile') || carImageryUrl;
            ...
            <span
              style={{
                ...
                ...(claimResult.status === 'verified'
                  ? { background: 'rgba(249,115,22,0.14)', border: '1px solid rgba(249,115,22,0.4)', color: 'var(--accent)' }
                  : claimResult.status === 'claimed'
                    ? { background: 'rgba(32,192,96,0.1)', border: '1px solid rgba(32,192,96,0.3)', color: '#20c060' }
                    : { background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.1)', color: 'var(--dim)' }),
              }}
            >
              {claimResult.status === 'verified' && <Shield className="w-3 h-3" />}
              {claimResult.status === 'claimed' && <Check className="w-3 h-3" />}
              {claimResult.status === 'unclaimed' && <Circle className="w-3 h-3" />}
              {claimResult.label}
            </span>
          </div>
        ...
        <div className="px-4 stg">
              ...
              <div className="flex-1">
                <h1 className="font-heading text-[22px] font-semibold text-white mb-1">
                  {vehicle.year || vehicle.make || vehicle.model
                    ? `${vehicle.year || ''} ${getVehicleDisplayName(vehicle).make} ${getVehicleDisplayName(vehicle).model}`.trim()
                    : 'Unknown Vehicle'}
                </h1>
            ...
            <div className="mb-4 space-y-2">
              ...
              <button
                ...
                style={{
                  ...
                  textTransform: 'uppercase',
                  ...(isTracking
                    ? { background: 'rgba(249,115,22,0.15)', border: '1px solid rgba(249,115,22,0.4)', color: 'var(--accent)' }
                    : { background: 'var(--accent)', border: 'none', color: '#030508' }),
                }}
              >
          ...
          {isUnclaimed && (
            <div className="mb-4 border border-white/[0.06] rounded-xl p-4">
              ...
              <p className="text-xs text-secondary text-center mt-2">Claiming lets you manage your plate profile and respond to spots</p>
            </div>
          )}
          ...
          {isOwner && (
            <>
              <div className="card-v3 card-v3-lift mt-6 p-4">
        ...
        <div id="reviews" className="card-v3 p-6 mx-4 mb-6">
          ...
          {reviews.length === 0 ? (
            <p className="text-[var(--t3)] text-center py-8">
              No spots yet. Spot this plate to leave the first one!
            </p>
          ) : (
