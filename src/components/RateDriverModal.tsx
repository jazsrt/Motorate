      ...
      if (existing) {
        const { error } = await supabase
          ...
          .update({
            ...
            ...(vehicleRating > 0 ? { rating_vehicle: vehicleRating } : {}),
            ...(comment.trim() ? { caption: comment.trim() } : {}),
          })
          .eq('id', existing.id);
        ...
        const { data: post, error } = await supabase
          ...
          .insert({
            ...
            spot_type: 'quick',
            rating_driver: driverRating,
            rating_driving: driverRating,
            rating_vehicle: vehicleRating || driverRating,
        ...
        if (error) {
          if (error.code === '23505') {
            showToast('You have already spotted this vehicle', 'error');
            return;
          }
          throw error;
        }
        ...
        if (post) {
          ...
          await awardReputationPoints(user.id, 'COMMENT_LEFT', 'review', post.id);
