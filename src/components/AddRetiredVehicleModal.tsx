...
export function AddRetiredVehicleModal({ userId, onClose, onSuccess }: AddRetiredVehicleModalProps) {
  ...
  const [fetchingStockImage, setFetchingStockImage] = useState(false);
  ...
  const handleUseStockImage = async (photoNumber: 1 | 2) => {
    if (!formData.make || !formData.model) {
      showToast('Enter make and model first', 'error');
      return;
    }

    setFetchingStockImage(true);
    ...
    try {
      ...
      if (imageUrl) {
        if (photoNumber === 1) {
          setFormData({ ...formData, photo_url_1: imageUrl });
        } else {
          setFormData({ ...formData, photo_url_2: imageUrl });
        }
        showToast('Stock image added', 'success');
      } else {
        showToast('No stock image found for this vehicle', 'error');
      }
    } catch (error) {
      console.error('Error fetching stock image:', error);
      showToast('Failed to fetch stock image', 'error');
    } finally {
      setFetchingStockImage(false);
    }
  };
        ...
        <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto p-4 space-y-4">
          ...
          <div>
            <div className="flex items-center justify-between mb-2">
              <label className="text-sm font-bold text-textsecondary">
                Photos (max 2)
              </label>
              {formData.make && formData.model && (
                <button
                  type="button"
                  onClick={() => handleUseStockImage(formData.photo_url_1 ? 2 : 1)}
                  disabled={fetchingStockImage || (!!formData.photo_url_1 && !!formData.photo_url_2)}
                  className="flex items-center gap-1.5 text-xs font-bold text-[#F97316] hover:text-[#fb923c] transition-colors disabled:opacity-40 disabled:cursor-not-allowed"
                >
                  <Car className="w-3.5 h-3.5" />
                  {fetchingStockImage ? 'Fetching...' : 'Use Stock Image'}
                </button>
              )}
            </div>
            <div className="grid grid-cols-2 gap-3">
              ...
              <div className="relative">
                <input
                  ...
                  className="hidden"
                  id="photo-1"
                  disabled={uploadingPhoto || fetchingStockImage}
                />
                <label
                  htmlFor="photo-1"
